...
> Unit testing is available from v1.93.0. 

You can use `vmctl` to run unit tests for alerting and recording rules.
vmctl will perform the following actions:
...
See how to run vmctl for unit test below:
```
# Run vmctl with one or multiple test files via --files cmd-line flag
./vmctl rule-unittest --files test1.yaml --files test2.yaml 
```

vmctl rule-unittest is compatible with [Prometheus config format for tests](https://prometheus.io/docs/prometheus/latest/configuration/unit_testing_rules/#test-file-format)
except `promql_expr_test` field. Use `metricsql_expr_test` field name instead. The name is different because vmctl 
...
# Enterprise version of vmctl supports S3 and GCS paths to rules.
rule_files:
  [ - <string> ]
...
With `rules.yaml` in the same directory, run `./vmctl rule-unittest --files=./unittest/testdata/test.yaml`.
